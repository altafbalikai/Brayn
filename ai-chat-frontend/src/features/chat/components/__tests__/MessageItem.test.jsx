...
vi.mock("../MessageVersions", () => ({
  MessageVersions: () => <div data-testid="message-versions" />,
}));
...
describe("MessageItem — edit mode", () => {
  ...
  it("dispatches editMessage thunk when Submit is clicked", async () => {
    const msg = {
      _id: MSG_ID_1,
      role: "user",
      text: "text",
  ...
  it("Submit and Cancel buttons are disabled when sending is true", () => {
    const msg = { _id: MSG_ID_1, role: "user", text: "text" };
    renderWithProviders(
      <MessageItem
        msg={msg}
...
describe("MessageItem — branch navigator", () => {
  ...
  it("renders navigator when branchMap has branches for this message", () => {
    ...
    const branchMap = {
      c1: [
        {
          branchConvId: "c2",
          editedMessageId: MSG_ID_1,
          branchedFromMessageId: MSG_ID_1,
        },
      ],
    };
  ...
  it("dispatches switchToBranch after fetching target branch on arrow click", async () => {
    ...
    await waitFor(() => {
      expect(conversationService.getConversation).toHaveBeenCalledWith("b1");
  ...
  it("right arrow is visually disabled at last index", () => {
    ...
    renderWithProviders(
      <MessageItem
        ...
        editingMessageId={null}
        branchMap={branchMap}
        currentConversationId="b1"
      />,
      {
        preloadedState: {
          currentConversation: { _id: "b1", parentConversationId: "root" },
        },
      },
    );
